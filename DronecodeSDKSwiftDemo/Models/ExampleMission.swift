...
class ExampleMission {
    ...
    init(){
        ...
        if missionItems.isEmpty {
            ...
            do {
                let entry = MissionItem(latitudeDeg: 47.398039859999997,
                                        longitudeDeg: 8.5455725400000002,
                                        ...
                                        gimbalYawDeg: 0.0,
                                        loiterTimeS: 0,
                                        cameraAction: CameraAction.startPhotoInterval)//CameraAction.takePhoto)
            ...
            do {
                let entry = MissionItem(latitudeDeg: 47.398039859999997,
                                        ...
                                        gimbalYawDeg: 0.0,
                                        loiterTimeS: 0,
                                        cameraAction: CameraAction.none)//CameraAction.startPhotoInterval)
                missionItems.append(entry)
            }
            do {
                let entry = MissionItem(latitudeDeg: 47.398036222362471,
                                        longitudeDeg: 8.5450146439425509,
                                        relativeAltitudeM: 10.0,
                                        speedMPS: 10.0,
                                        isFlyThrough: true,
                                        gimbalPitchDeg: -30.0,
                                        gimbalYawDeg: 0.0,
                                        loiterTimeS: 10.0,
                                        cameraAction: CameraAction.none)//CameraAction.stopPhotoInterval)
                missionItems.append(entry)
            }
            do {
                let entry = MissionItem(latitudeDeg: 47.397825620791885,
                                        longitudeDeg: 8.5450092830163271,
                                        relativeAltitudeM: 10.0,
                                        speedMPS: 10.0,
                                        isFlyThrough: true,
                                        gimbalPitchDeg: -60.0,
                                        gimbalYawDeg: 0.0,
                                        loiterTimeS: 0,
                                        cameraAction: CameraAction.none)//CameraAction.startVideo)
                missionItems.append(entry)
            }
            do {
                let entry = MissionItem(latitudeDeg: 47.397832880000003,
                                        ...
                                        gimbalYawDeg: 0.0,
                                        loiterTimeS: 0,
                                        cameraAction: CameraAction.stopPhotoInterval)//CameraAction.stopVideo)
                missionItems.append(entry)
            }
        }
